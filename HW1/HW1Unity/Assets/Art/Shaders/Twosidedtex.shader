    ...
    SubShader
    {
        ...
        Tags {"LightMode" = "ForwardAdd"}
            ...
            float4 frag(vertexShaderOutput i):SV_Target
            {
                ...
                if(i.vertInObjCoords.x < 0)
                    finalColor = float4(finalColor, 1.0) * tex2D(_Texture1, i.uv);
                else
                    finalColor = float4(finalColor, 1.0) * tex2D(_Texture2, i.uv);
