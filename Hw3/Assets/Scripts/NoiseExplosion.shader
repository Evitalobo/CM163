...
uniform float _DisplacementAmt;
    ...
    struct appdata
            ...
            struct v2f
            {
                    ...
                    float noise1 : TEXCOORD1;
                    float noise2 : TEXCOORD2;
            };
           ...
           { 
            ...
            float highFreq = -.5 * turbulence( 0.8 * (v.vertex.xyz + timeVec) );
           ...
           {
             float3 color = float3( 1.0 * ( 1.0 - (3.0 * i.noise1) ), 1.0, 1.0 );
             float3 color2 = float3( 1.0, .1 * ( 1.0 - (3.0 * i.noise2) ), 1.0 );
  
             //float4 finalColor = float4(i.noise2,i.noise2,i.noise2,1.0);
             //return finalColor;
             
             return float4( color.r, color2.gb, 0.0 );        
